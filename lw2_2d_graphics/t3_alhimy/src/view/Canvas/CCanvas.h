...
public:
		...
		: m_width(width), m_height(height)
	{
		...
		if (!m_font.loadFromFile("./fonts/Strogo.ttf"))
		{
	...
	void DrawImage(const std::string &filename, const Point &position, const Size &size) override
	{
		if (filename.empty())
		{
			return;
		...
		if (it == m_textureCache.end())
		{
			...
			auto result = m_textureCache.emplace(filename, std::move(texture));
			it = result.first;
		}
