...
{
public:
    const Size POPUP_SIZE = Size(300, 100);
    const Point POPUP_POSITION = Point(Constants::WIDTH / 2 - POPUP_SIZE.width / 2, Constants::HEIGHT / 2 - POPUP_SIZE.height / 2);
    const std::wstring TEXT = L"Игра завершена";

    Popup(
        ICanvas &canvas)
        : m_canvas(canvas),
          m_exitButton(ButtonData(
              L"ВЫЙТИ", {100, 50}, {100, 20}, 0xFF0000FF,
              [&canvas]()
              {
                  canvas.Exit();
              }))
    ...
    void Draw(unsigned &zIndex)
    {
        ...
        auto textSize = m_canvas.GetTextSize(TEXT, 24);
        m_canvas.SetColor(0x888888FF);
        m_canvas.DrawString(TEXT, {POPUP_POSITION.x + POPUP_SIZE.width / 2 - textSize.width / 2, POPUP_POSITION.y}, 24);
