...
class Mesh {
public:
    static MeshData Rect(Color color, std::vector<std::string> textures = {}, Size3D textureSize = {1, 1, 1})
    {
        MeshData meshData;
        SubMeshData mesh;

        float uvSize[2] = {textureSize.width, textureSize.height};
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            mesh.vertices.push_back({RECT_VERTEX_POSITIONS[vertexIndex], {0, 0, 1}, color, uv, RECT_FACE_UV[i]});
        }
    ...
    static MeshData Cube(Color color, std::vector<std::string> textures = {}, Size3D size = {1, 1, 1}) {
        ...
        for (int face = 0; face < 6; face++)
        {
            for (int i = 0; i < 4; i++)
            {
                ...
                mesh.vertices.push_back({CUBE_VERTEX_POSITIONS[vertexIndex], CUBE_FACE_NORMALS[face], color, uv, RECT_FACE_UV[i]});
            }
        }
