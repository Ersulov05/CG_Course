...
class Mesh {
public:
    static MeshData Flag(Color color)
    {
        MeshData meshData;
        SubMeshData mesh;
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            mesh.vertices.push_back({FLAG_VERTEX_POSITIONS[vertexIndex], {0, 0, 1}, color, uv, uv});
        }
...
private:
    inline static Point3D FLAG_VERTEX_POSITIONS[4] = {
        Point3D(-1, -0.5, 0), 
        Point3D(1, -0.5, 0),  
        Point3D(1, 0.5, 0),   
        Point3D(-1, 0.5, 0)  
