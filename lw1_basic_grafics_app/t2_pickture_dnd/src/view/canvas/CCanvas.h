...
public:
	...
	using ProcessEventsCallback = std::function<void(sf::Event &)>;
	...
	void RunWindow(UpdateCallback updateCallback, ProcessEventsCallback processEventsCallback)
	{
		...
		while (window.isOpen())
		{
			ProcessEvents(window, processEventsCallback);
...
private:
	...
	void ProcessEvents(sf::RenderWindow &window, ProcessEventsCallback processEventsCallback)
	{
		sf::Event event;
